...
When('I fill in the password field with {string}', async (password: string) => {
  await pageFixture.page.getByPlaceholder('Введите пароль').fill(password);
});

Then('I see the error message {string}', async (expectedErrorMessage: string) => {
...
Then('I see the message {string}',async (expectedMessage:string) => {
